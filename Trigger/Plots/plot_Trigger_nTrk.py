...
parser.add_argument("-d", "--data1", default="test_Run2023C_trigger.root", help="The rootfile for 2023 data") 
...
leg.AddEntry(gra_jetpt, "Run2023C", "pe")
...
# --- Build ratio graph (data / MC) with errors ---
n_data = gra_jetpt.GetN()
n_mc   = gra_jetpt_mc.GetN()
npoints = min(n_data, n_mc)

ratio = ROOT.TGraphErrors(npoints)
ratio.SetName("ratio")
...
for i in range(npoints):
    ...
    if ym_val > 0 and yd_val > 0:
        ...
        ratio.SetPoint(i, xd_val, R)
        ratio.SetPointError(i, 0.0, err)
    else:
        ratio.SetPoint(i, xd_val, 0.0)
...
ratio.GetXaxis().SetTitle("Offline PF jet p_{T} [GeV]")
ratio.GetXaxis().SetTitleSize(0.12)
ratio.GetXaxis().SetLabelSize(0.1)
ratio.GetXaxis().SetRangeUser(0.0,140.0)
...
line = ROOT.TLine(0.0, 1.0, 140.0, 1.0)
line.SetLineStyle(2)
line.Draw("SAME")
...
can.SaveAs("Run2023C-trigger_CalojetPt40_Eff.png")
can.SaveAs("Run2023C-trigger_CalojetPt40_Eff.pdf")
...
# Adjust pads (top for efficiency, bottom for ratio)
pad1_2 = can2.cd(1)
...
leg.AddEntry(gra_PromptTrk, "Run2023C", "pe")
...
ratio.GetXaxis().SetTitle("Number of Offline Prompt Tracks")
ratio.GetXaxis().SetTitleSize(0.12)
ratio.GetXaxis().SetLabelSize(0.1)
ratio.GetXaxis().SetRangeUser(-0.5,20.0)
...
# Draw reference line at ratio=1
line = ROOT.TLine(-0.5, 1.0, 20.0, 1.0)
line.SetLineStyle(2)
line.Draw("SAME")
...
can2.SaveAs("Run2023C-trigger_PromptTrack_Eff.png")
can2.SaveAs("Run2023C-trigger_PromptTrack_Eff.pdf")
...
leg.AddEntry(gra_DisplacedTrk, "Run2023C", "pe")
...
ratio.GetXaxis().SetTitle("Number of Offline Displaced Tracks")
ratio.GetXaxis().SetTitleSize(0.12)
ratio.GetXaxis().SetLabelSize(0.1)
ratio.GetXaxis().SetRangeUser(-0.5,12.5)
...
# Draw reference line at ratio=1
line = ROOT.TLine(-0.5, 1.0, 12.5, 1.0)
line.SetLineStyle(2)
line.Draw("SAME")

can3.Update()

can3.SaveAs("Run2023C-trigger_DisplacedTrack_Eff.png")
can3.SaveAs("Run2023C-trigger_DisplacedTrack_Eff.pdf")
